...
static PyObject *PyLWPR_G_num_rfs(PyLWPR *self, void *closure) {
   ...
   matout = (PyArrayObject *) PyArray_NewFromDescr(&PyArray_Type,
      PyArray_DescrFromType(NPY_INT), 1, &len, NULL, NULL, 1, NULL);
      
   for (i=0;i<len;i++) {
      int *di = (int *) matout->data + i*PyArray_STRIDE(matout,0);
      *di = self->model.sub[i].numRFS;
   }
   return (PyObject *) matout;
}
...
static PyObject *PyLWPR_update_maxw(PyLWPR *self, PyObject *args) {
   ...
   return Py_BuildValue("(O,O)",
      get_array_from_vector(model->nOut, self->extra_out2),
      get_array_from_vector(model->nOut, self->extra_out3));
}
...
static PyObject *PyLWPR_predict_conf(PyLWPR *self, PyObject *args) {
   ...
   return Py_BuildValue("(O,O)",
      get_array_from_vector(model->nOut, self->extra_out),
      get_array_from_vector(model->nOut, self->extra_out2));   
}
...
static PyObject *PyLWPR_predict_J(PyLWPR *self, PyObject *args) {
   ...
   return Py_BuildValue("(O,O)",
      get_array_from_vector(model->nOut, self->extra_out),
      get_array_from_matrix(model->nOut, model->nOut, model->nIn, self->extra_J));      
}
